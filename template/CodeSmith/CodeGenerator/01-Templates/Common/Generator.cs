    ...
    public class Generator
    {
        ...
        private void CreateProperties(Entity entity, TableSchema tableSchema)
        {
            foreach (ColumnSchema columnSchema in tableSchema.Columns)
            {
                ...
                if(DefaultValue(columnSchema).Length > 0)
                {
                    property.IsHasDefault = true;
                    property.DefaultValue = DefaultValue(columnSchema);
                }
        ...
        private static string DefaultValue(DataObjectBase column)
        {
            try
            {                
                var value = column.ExtendedProperties[ExtendedPropertyNames.DefaultValue].Value.ToString();
                value = value.Replace("(NULL)","").Replace("(","").Replace(")","").Replace("'","");
                return value;
            }
            catch (Exception ex)
            {
