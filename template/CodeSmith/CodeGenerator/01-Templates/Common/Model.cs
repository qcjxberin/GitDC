...
namespace SchemaMapper
{
    ...
    public class Entity : EntityBase
    {
        ...
        public string GetAggregateRoot()
        {
            ...
            if (GetKeyType() == typeof(short))
                return string.Format("AggregateRoot<{0},short>", ClassName);
    ...
    public class Property : EntityBase
    {
        ...
        public bool IsHasDefault { get; set; }
        public bool IsHasDefaultSpecified
        {
            get { return IsHasDefault; }
        }
        ...
        private void ValidateRequired(List<string> result)
        {
            ...
            {
                result.Add(string.Format("[Required(ErrorMessage = \"{0}不能为空\")]", ColumnName));
                return;
            } 
            if(Description.IndexOf('(') > -1)
            {
                result.Add(string.Format("[Required(ErrorMessage = \"{0}不能为空\")]", Description.Substring(0,Description.IndexOf('('))));
            }
            else
            {
        ...
        private void ValidateStringLength(List<string> result)
        {
            ...
            if (Description.Trim().Length == 0)
            {
                result.Add(string.Format("[StringLength( {0}, ErrorMessage = \"{1}输入过长，不能超过{0}位\" )]", MaxLength, ColumnName));
                return;
            } 
            if(Description.IndexOf('(') > -1)
            {
                result.Add(string.Format("[StringLength( {0}, ErrorMessage = \"{1}输入过长，不能超过{0}位\" )]", MaxLength, Description.Substring(0,Description.IndexOf('('))));
            }
            else
            {
        ...
        private void GetColumnName(List<string> result)
        {
            result.Add(string.Format("[Column( \"{0}\" )]", ColumnName));
        }
